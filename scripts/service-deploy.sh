...
case $ENVIRONMENT in
    prod|production)
        ...
        ENV=prod
        BRANCH=main
        export GRAFANA_ROOT_URL="https://alpha-live.quantus.kr/grafana"
        ;;
    stage|staging)
        ENV_FILE=.env
        ENV=stage
    ...
    dev|development)
        ...
        ENV=dev
        BRANCH=dev
        export GRAFANA_ROOT_URL="https://alpha-dev.quantus.kr/grafana"
        ;;
    *)
...
server {
    ...
    location /grafana/ {
        ...
        proxy_set_header X-Grafana-Root-Url ${GRAFANA_ROOT_URL};
