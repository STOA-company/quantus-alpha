...
class NewsService:
    ...
    def get_etf_news_detail(
        ...
        unique_tickers = [info[0] for info in stock_tickers]
        ...
        tickers_tuple = f"({', '.join([f"'{ticker}'" for ticker in unique_tickers])})"
        query = f"""
            SELECT EXISTS (
                SELECT 1
        ...
        stock_info = self.db._execute(text(query)).scalar()
        if df_news.empty:
            if stock_info:
