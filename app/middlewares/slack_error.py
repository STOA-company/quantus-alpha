...
class SlackExceptionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            ...
            if (
                response.status_code in self.error_status_codes
                and self.environment in self.notify_environments
            ):
    ...
    async def _send_slack_notification(
        ...
        try:
            ...
            # 스택 트레이스 (설정된 경우만)
            stack_trace = None
            if self.include_traceback and exception:
