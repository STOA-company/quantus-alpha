...
HTML 템플릿과 PDF/Excel 등 첨부 파일을 포함한 이메일 전송 기능을 제공합니다.

사용 예시:
    # 1. notification.html 템플릿 사용 (간편한 방법)
    template = create_notification_email(
        greeting="안녕하세요!",
        content="포트폴리오 리포트가 준비되었습니다. 첨부된 PDF를 확인해주세요.",
        closing="감사합니다."
    )
    ...
    # 1-1. 수동으로 템플릿 처리하는 방법
    with open('static/templates/email/notification.html', 'r', encoding='utf-8') as f:
        template = f.read()
    
    # 플레이스홀더 치환
    template = insert_string_into_html(template, "{{ greeting }}", "안녕하세요!")
    template = insert_string_into_html(template, "{{ content }}", "포트폴리오 리포트가 준비되었습니다.")
